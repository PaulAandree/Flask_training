...
df_crecimiento_real = pd.read_csv(
    "df_crecimiento_real.csv",
    parse_dates=True,
    index_col="datetime",
    names=["datetime", "Crecimiento real por trimestre"],
)
df_crecimiento_real_total = pd.read_csv(
    "df_crecimiento_real_total.csv",
    parse_dates=True,
    index_col="datetime",
    names=["datetime", "Crecimiento real total"],
)
...
if page == "Dashboard":
    ...
    fig.add_annotation(
        ...
        font={
            "size": 20,
            "color": "#00FF00" if cambio_porcentual >= objetivo else "#FF0000",
        },
    )
    ...
    indicator = go.Indicator(mode="number+delta", value=0)
    fig = go.Figure(data=[indicator])
    fig.update_layout(height=200, width=200, margin=dict(t=0, b=0, l=0, r=0))
    ...
    fig = px.line(
        df_internet[geography], title="Acceso a Internet cada 100 hogares: " + geography
    )
    fig.update_layout(
        showlegend=False,
        xaxis_title="Año y trimestre",
        yaxis_title="Acceso a Internet cada 100 hogares",
    )
    ...
    bar = go.Bar(
        ...
        marker=dict(
            color=[
                "rgba(63, 195, 128, 1)" if x > 0 else "rgba(219, 10, 91, 1)"
                for x in df_crecimiento_real["Crecimiento real por trimestre"]
            ],
    ...
    fig.update_layout(
        title="Crecimiento real a nivel Nación de ingresos por Internet fijo (quitando inflación, expresado en %)",
        xaxis_title="Año y trimestre",
    ...
    bar = go.Bar(
        ...
        marker=dict(
            color=[
                "rgba(63, 195, 128, 1)" if x > 0 else "rgba(219, 10, 91, 1)"
                for x in df_crecimiento_real_total["Crecimiento real total"]
            ],
            line=dict(color="rgb(0,0,0)", width=1.5),
        ),
    ...
    fig = go.Figure(data=[bar])
    fig.update_layout(
        title="Crecimiento real a nivel Nación de ingresos de Telecomunicaciones (quitando inflación, expresado en %)",
        xaxis_title="Año y trimestre",
        yaxis_title="Crecimiento real (%)",
    )
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.line(
        df_crecimiento_real,
        title="Crecimiento real (quitando inflación) de ingresos por Internet fijo (%)",
    )
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
#    col1,col2 = st.columns(2)
